...
#define MIN_THINK 1
#define MAX_THINK 100000
#define MIN_DINE 1
#define MAX_DINE 100000
...
int main(int argc, char *argv[]) {
    ...
    // Initialization of chopsticks
    int i;
    for (i = 0; i < number_of_philosophers; i++) {
        pthread_mutex_init(&chopsticks[i], NULL);
    }

    // Creating of threads for philosophers
    ...
    // Waiting for all philosophers to finish dining
    for (i = 0; i < number_of_philosophers; i++) {

        pthread_join(philosophers[i], NULL);
    }

    // Calculate mean of hungry time
    double total_hungry_time = 0;
    for (i = 0; i < number_of_philosophers; i++) {

        total_hungry_time += hungry_time[i];
    }
    double average_hungry_time = total_hungry_time / number_of_philosophers;
    ...
    double standard_deviation = sqrt(sum_of_squared_differences / number_of_philosophers);



    // Print results
    printf("Mean time of hungry: %f\n", average_hungry_time);
